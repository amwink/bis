...
typedef struct filestats { // make a struct:
    ...
    float slipos;          //		float	slice position
} filestats;
...
std::vector<filestats> bisdicom<value_type>::get_seriesfiledata ( std::string dirname ) {
	...
	if ( !added_files ) { // use first file to populate the info record
		...
		superclass::sidecar["PhaseEncodingDimension"] = ( superclass::sidecar["SlicesDimension"] > 0 ) ? 0 : 1;
	    case 'C': // column direction ( 1 in slice ), i.e. 1 if slice direction == 2, 2 otherwise
		superclass::sidecar["PhaseEncodingDimension"] = ( superclass::sidecar["SlicesDimension"] > 1 ) ? 1 : 2;
	    }
...
std::vector<std::string> bisdicom<value_type>::get_intensitydata ( std::vector<filestats>& series_filedata ) {
	...
	for ( auto startpos = ( series_filedata[0] ).slipos; sli < series_filedata.size (); sli++ )
		if ( ( series_filedata[sli] ).slipos == startpos ) break;
	...
	superclass::sidecar["RepetitionTime"] =
		( ( series_filedata[2 * sli] ).aqtime - ( series_filedata[0] ).aqtime ) / 2.;
	...
	if ( ( unsigned ( superclass::sidecar["Slices"] ) == sli ) && ( num_files == ( sli * vol ) ) ) {
		...
		if ( sli > 1 ) {
			...
			if ( vol > 1 ) {
				...
				superhyper::newsizes ( { im_size[0], im_size[1], im_size[2], im_size[3] } );
	...
	// do a check of the data array -- continue if this is sound
